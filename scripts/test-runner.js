...
function execExternal(externalSpec, code) {
  ...
  try {
    ...
    if (inspectResult instanceof Promise) {
        ...
        return () => SerialPromises(
          flagPermutations
            ...
            .map(function([delayInitializations, inlineExpressions, lazyObjectsRuntime]) {
              ...
              return () => runTest(test.name, test.file, options, args).then(testResult => {
                if (testResult) {
...
function main(): number {
  ...
  (args.cpuprofilePath ? runWithCpuProfiler : run)(args).then(function(result) {
    if (!result) {
      process.exit(1);
    } {
      process.exit(0)
